...
public class AutomaticIssuesLoader : Shelf, ILoadShelf
{
    ...
    private async void LoadGitHubIssues()
    {
        GitHubShelfConfiguration gitHubShelfConfiguration = new GitHubShelfConfiguration("rwth-acis", "VIAProMa");
        ApiResult<Issue[]> apiResult = await GitHub.GetIssuesInRepository(gitHubShelfConfiguration.Owner, gitHubShelfConfiguration.RepositoryName, page, issuesMultiListView.numberOfItemsPerListView * issuesMultiListView.NumberOfListViews);
        while (apiResult.HasError)
        {
            apiResult = await GitHub.GetIssuesInRepository(gitHubShelfConfiguration.Owner, gitHubShelfConfiguration.RepositoryName, page, issuesMultiListView.numberOfItemsPerListView * issuesMultiListView.NumberOfListViews);
        }
        List<Issue> items = new List<Issue>(apiResult.Value);
        issuesMultiListView.Items = items;
    }
